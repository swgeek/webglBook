...
function main()
{
   ...
   var fogDistance = new Float32Array([55, 80]);
   var eye = new Float32Array([25, 65, 35, 1]);
   ...
   var u_FogDistance = gl.getUniformLocation(gl.program, 'u_FogDistance');
   if (! u_MvpMatrix || ! u_ModelMatrix || ! u_Eye || !u_FogColor || ! u_FogDistance) throw 'err';

   gl.uniform3fv(u_FogColor, fogColor);
   gl.uniform2fv(u_FogDistance, fogDistance);
   ...
   document.onkeydown = function(ev) { keydown(ev, gl, n, u_FogDistance, fogDistance); };

   gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
   gl.drawElements(gl.TRIANGLES, n, gl.UNSIGNED_BYTE, 0);

   // what the hell is this? From sample code, but don't see the point right now
   // comment it out later and see the difference
